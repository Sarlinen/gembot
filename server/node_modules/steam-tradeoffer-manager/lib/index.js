...
TradeOfferManager.prototype.getOffer = function(id, callback) {
	this._apiCall('GET', 'GetTradeOffer', 1, {"tradeofferid": id}, (err, body) => {
		...
		let rawOffer = Helpers.sanitizeRawOffer(body.response.offer);
...
TradeOfferManager.prototype.getOffers = function(filter, historicalCutoff, callback) {
	...
	var finish = () => {
		...
		sentOffers = sentOffers.map(offer => Helpers.sanitizeRawOffer(offer));
		receivedOffers = receivedOffers.map(offer => Helpers.sanitizeRawOffer(offer));
